...
type FetchPackageInfoHook = (
  pkg: Package,
  extra: Set<string>,
  registerData: (
    namespace: string,
    data:
      | Array<formatUtils.Tuple>
      | { [key: string]: formatUtils.Tuple | undefined },
  ) => void,
) => Promise<void>;

// eslint-disable-next-line arca/no-default-export
export class InfoEnhancedCommand extends BaseCommand {
  static paths = [[`info-enhanced`]];
  ...
  prodOnly = Option.Boolean(`--production`, false, {
    description: `Print versions of production packages only`,
  });

  noConditions = Option.Boolean(`--no-conditions`, false, {
    description: `Filter out packages with conditions`,
  });
  ...
  async execute() {
    ...
    const traverseFromHashes = (
      initialHashes: Array<LocatorHash>,
      { recursive }: { recursive: boolean },
    ) => {
      const seen = new Map<LocatorHash, Package>();
      const pass = [...initialHashes];
      ...
      while (pass.length > 0) {
        ...
        if (!recursive && initialHashes.includes(hash)) continue;
    ...
    const traverseWorkspace = (
      workspace: Workspace,
      { recursive, prodOnly }: { recursive: boolean; prodOnly: boolean },
    ) => {
      if (prodOnly) {
        const storedPackageLocatorsByScopeName = [
          ...project.storedPackages.values(),
        ].reduce(
          (acc, pkg) => {
            const key = pkg.scope ? `@${pkg.scope}/${pkg.name}` : pkg.name;
            return {
              ...acc,
              [key]: [...(acc[key] || []), pkg.locatorHash],
            };
          },
          {} as Record<string, LocatorHash[]>,
        );

        const locators = [...workspace.manifest.dependencies.values()]
          .map(
            (dep) =>
              storedPackageLocatorsByScopeName[
                dep.scope ? `@${dep.scope}/${dep.name}` : dep.name
              ],
          )
          .flat()
          .filter((locator) => locator !== null) as LocatorHash[];

        return traverseFromHashes(locators, { recursive });
      }

      const initialHash = workspace.anchoredLocator.locatorHash;
      return traverseFromHashes([initialHash], { recursive });
    };
    ...
    const traverseAllWorkspaces = ({
      ...
      for (const workspace of project.workspaces)
        for (const pkg of traverseWorkspace(workspace, { recursive, prodOnly }))
    ...
    const getLookupSet = ({
      ...
      if (all) {
        return traverseAllWorkspaces({ recursive, prodOnly });
      } else {
        return traverseWorkspace(workspace!, { recursive, prodOnly });
      }
    };
    ...
    const findSelectedSet = ({
      ...
      const lookupSet = getLookupSet({ all, recursive, prodOnly });

      const matchers = this.patterns.map((pattern) => {
        const patternLocator = structUtils.parseLocator(pattern);
        const identRegex = mm.makeRe(
          structUtils.stringifyIdent(patternLocator),
        );
        ...
        return (pkg: Package) => {
          ...
          if (
            patternIsVirtual &&
            pkgIsVirtual &&
            patternLocator.reference !== pkg.reference
          )
            return false;
    ...
    const { selection, sortedLookup } = findSelectedSet({
      ...
      prodOnly: this.prodOnly,
      noConditions: this.noConditions,
    });
    ...
    const builtinInfoBuilders: Array<Exclude<FetchPackageInfoHook, undefined>> =
      [
        // Manifest fields
        async (pkg, extra, registerData) => {
          ...
          registerData(`Manifest`, {
            [`License`]: formatUtils.tuple(
              formatUtils.Type.NO_HINT,
              manifest.license,
            ),
            [`Homepage`]: formatUtils.tuple(
              formatUtils.Type.URL,
              manifest.raw.homepage ?? null,
            ),
          });
        },
    ...
    for (const pkg of selection) {
      ...
      if (!isVirtual) {
        ...
        await configuration.triggerHook(
          (hooks: { fetchPackageInfo?: FetchPackageInfoHook }) => {
      ...
      if (pkg.dependencies.size > 0 && !isVirtual) {
        registerData(
          ...
          [...pkg.dependencies.values()].map((dependency) => {
            ...
            const resolution =
              typeof resolutionHash !== `undefined`
